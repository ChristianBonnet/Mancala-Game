...
import ServerCommunication.GameEvent;
import ServerCommunication.GameState;
...
public class Player extends Socket {
    ...
    public GameState waitForMove() throws IOException, ClassNotFoundException {
        ...
        int otherPitsSum = 0;
        for (int pitCounts : opponentsPits) {
        	otherPitsSum += pitCounts;
        }

        if (yourPitsSum == 0 || otherPitsSum == 0) {
            // game over
            gameOver = true;
